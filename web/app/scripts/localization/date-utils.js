(function() {
    ...
    function DateLocalization(LanguageState, WebConfig) {
        var languageMap = {
            '': {
                formats: {
                    ...
                    'numeric': 'mm/dd/yyyy',
                    'short': 'M Y',
                    'time': 'HH:mm:ss' // Ajout du format 'time' par défaut
                }
            },
            'af': {
                formats: {
            ...
            'en-us': {
                formats: {
                    ...
                    'numeric': 'm/dd/yyyy',
                    'short': 'M Y',
                    'time': 'HH:mm' // Format heure pour le français (sans secondes)
                },
                language: 'en'
            },
            'eo': {
        ...
        function getLocalizedDateString(date, format, includeTime, excludeSeconds) {
            ...
            // Créer l'objet convertedDate à partir de la bibliothèque jQuery Calendars
            var convertedDate = $.calendars.instance(conversion.calendar, conversion.language)
                .fromJSDate(localizedDate);
            
            // Convertir convertedDate en un objet JavaScript Date natif
            var jsDate = convertedDate.toJSDate();
            
            // Utiliser toLocaleDateString pour formater la date
            var datestring = jsDate.toLocaleDateString(conversion.language, {
                year: 'numeric',
                month: 'long',
                day: 'numeric'
            });
        
            // Si on inclut l'heure, ajouter l'heure formatée
            if (includeTime) {
