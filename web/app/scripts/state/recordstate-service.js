...
(function () {
    ...
    function RecordState($log, $rootScope, $q, localStorageService,
        ...
        function updateOptions(params) {
            ...
            return RecordTypes.query(filterParams).$promise.then(function(results) {
                ...
                if (!results.length) {
                    ...
                    if (options.length > 0) {
                        if (!selected) {
                            svc.setSelected(options[0]);  // Toujours sélectionner le premier si rien n'est sélectionné
                        } else if (!selected || !_.some(options, { uuid: selected.uuid })) {
                            svc.setSelected(options[0]);  // Sélectionne un élément valide si l'ancien n'existe plus
                        }
                    } else {
                        $log.warn('Aucune option disponible dans RecordState.');
                        selected = null; // Évite d'envoyer `undefined` dans setSelected
                    }
                }
            });
        }
        ...
        function setSelected(selection) {
            ...
            if (selection && selection.uuid && _.find(options, { uuid: selection.uuid })) {
                selected = selection;
